...
tif_paths = glob.glob("Average*.tif")
tif_paths.sort(key=os.path.getmtime)
expos = [7.272, 3.636, 1.818, 0.909, 0.455, 0.228]
Z, B, raw_imgs = pixelValues(tif_paths[0:5], expos, 30000)
g, lE = gsolve_scipy(Z, B, 2, w)
HDR_pic = construct_HDR(g, raw_imgs, expos)
HDR_pic1 = Image.fromarray(HDR_pic)
HDR_pic1.save("HDR_Calreticulin_Blank.tif")

Z, B, raw_imgs = pixelValues(tif_paths[6:11], expos, 30000)
g, lE = gsolve_scipy(Z, B, 2, w)
HDR_pic = construct_HDR(g, raw_imgs, expos)
HDR_pic1 = Image.fromarray(HDR_pic)
HDR_pic1.save("HDR20_Calreticulin_Signal.tif")
